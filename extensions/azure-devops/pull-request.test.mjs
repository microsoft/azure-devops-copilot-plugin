...
const repository = {
    ...
    name: "playground",
    webUrl: "https://dev.azure.com/contoso/agency/_git/playground",
};
...
function rawPullRequest(overrides = {}) {
    return {
        ...
        _links: { web: { href: "https://dev.azure.com/contoso/agency/_git/playground/pullrequest/4242" } },
        ...overrides,
    };
}
...
test("mapPullRequest maps a raw Azure DevOps payload", () => {
    ...
    assert.equal(mapped.repository, "playground");
    assert.equal(mapped.repositoryId, repository.id);
    assert.equal(mapped.createdBy, "Test Author");
    assert.equal(mapped.webUrl, "https://dev.azure.com/contoso/agency/_git/playground/pullrequest/4242");
...
test("mapPullRequest is idempotent", () => {
    ...
    assert.equal(twice.repository, "playground");
    assert.equal(twice.repositoryId, repository.id);
    assert.equal(twice.webUrl, "https://dev.azure.com/contoso/agency/_git/playground/pullrequest/4242");
...
test("parsePullRequestUrl reads dev.azure.com and visualstudio.com urls", () => {
    assert.deepEqual(parsePullRequestUrl("https://dev.azure.com/contoso/agency/_git/playground/pullrequest/4242"), {
        organization: "contoso",
        project: "agency",
        repository: "playground",
        id: 4242,
    });
    assert.deepEqual(parsePullRequestUrl("https://contoso.visualstudio.com/agency/_git/playground/pullrequest/7"), {
        organization: "contoso",
        project: "agency",
        repository: "playground",
        id: 7,
...
test("parsePullRequestUrl rejects unusable urls", () => {
    ...
    assert.equal(parsePullRequestUrl("https://github.com/contoso/playground/pull/4242"), null);
    assert.equal(parsePullRequestUrl("https://dev.azure.com/contoso/agency/_git/playground/pullrequest/0"), null);
});
