...
test("the pull request header is title and merge summary, with no external link competing", describeDom, async () => {
    const { window } = await boot({ homePrs: [101] });
    await openFromHome(window, "!101");
    ...
    assert.equal(copy?.lastElementChild?.className, "pr-merge-summary", "the title and merge summary stay grouped");
    assert.deepEqual(
        [...copy.querySelector(".pr-merge-summary").children].map((node) => node.className),
        ["pr-identity-summary", "pr-branch-flow"],
        "metadata and branch flow wrap as deliberate groups",
    );
    assert.ok(copy.querySelector(".pr-identity-summary .pr-proposal"));
    assert.equal(copy.querySelectorAll(".pr-branch-flow .pr-branch").length, 2);
    ...
    assert.match(styles, /@container \(max-width: 520px\)[\s\S]*?\.pr-header-actions \{[^}]*justify-content: flex-start/);
});
...
test("the repository is saved even when the engine raises no input event for it", describeDom, async () => {
    ...
    const state = {
        remote: { isAzureDevOps: false },
        connections: [{ source: "last-used", organization: "1esgitops", project: "agency", repositoryId: "", isDefault: false, isRemote: false, requiresProject: false }],
        repositories: ["agency", "other-repo"],
    };
    const { window } = await boot(state);
    window.document.getElementById("connectionsButton").click();
    await settle();
...
test("a changed field is saved when the engine raises change but not input", describeDom, async () => {
    const state = {
        remote: { isAzureDevOps: false },
        connections: [{ source: "last-used", organization: "1esgitops", project: "agency", repositoryId: "agency", isDefault: false, isRemote: false, requiresProject: false }],
    };
    const { window } = await boot(state);
    window.document.getElementById("connectionsButton").click();
    await settle();
...
test("the missing-remote note is informational, and says what is actually missing", describeDom, async () => {
    const { window } = await boot({
        remote: { isAzureDevOps: false, remoteName: "origin", remoteUrl: "https://github.com/agency-microsoft/playground.git" },
    ...
    assert.ok(
        note.textContent.includes("(github.com/agency-microsoft/playground)"),
...
test("the lifecycle decisions sit on the title's row, with the edit pencil beside them", describeDom, async () => {
    ...
    assert.equal(header?.lastElementChild, actions, "the actions are the rightmost item in the header");
    assert.equal(
    ...
    assert.match(styles, /\.pr-header-actions \{[^}]*margin-left: auto/);
});
