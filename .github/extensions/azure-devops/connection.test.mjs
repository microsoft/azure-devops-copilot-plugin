...
test("a repository change is stored and resolved back", async () => {
    const { home } = withHome();
    const { resolveConnections, writeConnectionPreference } = await loadConnection(home);
    writeConnectionPreference({ organization: "1esgitops", project: "agency", repositoryId: "agency" });
    assert.equal(resolveConnections({ remote: null })[0].repositoryId, "agency");
    writeConnectionPreference({ organization: "1esgitops", project: "agency", repositoryId: "other-repo" });
    const connections = resolveConnections({ remote: null });
