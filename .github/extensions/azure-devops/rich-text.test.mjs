...
test("corpus: snake_case placeholders keep their underscores", { ...needsDom, todo: "intraword emphasis" }, () => {
    const html = markdownToHtml("Events are named agency_<entrypoint>_<subject>_<action> by convention.");
    assert.ok(html.includes("agency_&lt;entrypoint&gt;_&lt;subject&gt;_&lt;action&gt;"), html);
});
...
test("a bot comment keeps its markup", needsDom, () => {
    const comment = [
        ...
        '<table><tr><td>App</td><td><strong>AgencyInfra</strong></td></tr></table>',
...
test("lone tags outside the allow-list are escaped so their text stays visible", needsDom, () => {
    for (const tag of ["repo", "org", "skillname", "String", "AgencySigningKeyProvider"]) {
...
test("sanitizer", needsDom, async (t) => {
    ...
    await t.test("bot markup still renders as elements", () => {
        const node = render('<table><tr><td><strong>AgencyInfra</strong></td></tr></table>', { format: "markdown" });
